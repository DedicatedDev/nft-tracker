...
function opAddNewContract(contract) {
    return new ft3_lib_1.Operation("bridge.add_contract", contract.chain, contract.pubkey.toString(), contract.type);
}
exports.opAddNewContract = opAddNewContract;
function opTransferOwnerShip(contract, tokenId, owner) {
    return new ft3_lib_1.Operation("bridge.transfer_ownership", contract.chain, contract.pubkey.toString(), tokenId, owner.toString());
}
